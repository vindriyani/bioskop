...
$user = $_POST['userid'];
$pass = $_POST['password'];
$cekuser = mysql_query("SELECT * FROM ibc WHERE userid = '$user'");
if ( mysql_num_rows($cekuser) <> 0 ) {
    ...
    if ( !$nama || !$mail || !$user || !$pass ) {
        ...
        echo '<a href="contact2.php">&laquo; Back</a>';
    } else {
        $simpan = mysql_query("INSERT INTO anggota VALUES('$nama','$mail','$user','$pass')");
        if ($simpan) {
            echo 'Pendaftaran Berhasil, Silahkan <a href="contact.php">Login</a>';
        } else {
            echo 'Proses Gagal!';
        }
    }
